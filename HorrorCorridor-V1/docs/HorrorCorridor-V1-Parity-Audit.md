...
- Progress note (2026-07-17): Reconciled the 612-line objective with the canonical natural-domain graph in `docs/HorrorCorridor-Natural-Domain-Execution-Reconciliation-2026-07-17.md`: 359 state owners, 428 unique behavior contracts, 132 closed contracts, and 296 open contracts. The old ordered Anomaly objective is explicitly superseded by the later monster-encounter, Monster Index, score, room-offer, and endless-building direction rather than silently mapped into unrelated domains. `SCENE-002` remains `Partially Fixed`: the furnished room has physical masonry, bounded wet response, a readable generator, connected structural ceiling overlap, wet-concrete floor identity, fine radial ceiling fracture, and small settled rubble, but wall/prop/ceiling material richness, true reflection, remaining silhouettes, and target-level edge density remain below the V2 reference.
- Progress note (2026-07-17): Fixed the bounded same-origin Shared Expedition reconnect/recovery slice. A headed two-page proof now moves the client, disconnects it, preserves its last place, keeps host authority advancing, shows readable Connection Lost and Recovering states, correlates `request-sync` to an authoritative recovery snapshot, restores the same room/game/seed/player/position, records Nexus Rejoining and Shared Chronicle history, and proves movement after recovery. All 13 gates pass with zero console errors; cross-device PeerJS remains unverified.
- Progress note (2026-07-17): Advanced the next bounded `SCENE-002` slice without creating a new domain or reflection core. Four standing-water and two wet-ground patches now use physical clearcoat response and six boundary-matched broken-light masks; the retained three-view wet proof passes movement, downward/side look, readability, no-HUD, reset/replay, and console gates. Detached cyan scratches and broad glowing-paint iterations were rejected. The refreshed canonical proof and `35,557 ms` Building-3 success path also pass. Status remains `Partially Fixed` because this is stylized broken-light response rather than true reflected environment, and material, prop, ceiling, and edge-density gaps remain.
- Progress note (2026-07-17): Fixed the bounded broken-generator silhouette through the existing object/shape/fidelity chain. Generic indexed cylinders now live in the wound-mesh owner; the generator composes 27 parts / 708 triangles, and the room only places/scales it. The first oversized boiler-like version was rejected. Dedicated generator, canonical, wet, and `32,864 ms` Building-3 success proofs pass; overall `SCENE-002` stays `Partially Fixed` for material, reflection, ceiling, remaining silhouettes, and edge density.
- Progress note (2026-07-17): Fixed the bounded structural ceiling/wall-overlap slice without creating a new natural domain or duplicating the Nexus object/shape/fidelity cores. The shared wound-mesh seam gained a closed convex-slab primitive, and `collapsed-ceiling-object-kit` composes 10 parts / 112 triangles into one wall-rooted irregular collapse. Four mechanical box/wedge iterations were rejected and removed. The dedicated three-view headed proof passes movement, upward look, readability, no-HUD, reset/replay, and console gates. `SCENE-002` remains `Partially Fixed` because fine fracture/rubble, material identity, true reflection, remaining silhouettes, and target edge density remain below the V2 reference.
- Progress note (2026-07-17): Fixed the bounded wet-concrete floor-material slice without creating a material taxonomy, new Nexus core, alias kit, or renderer-owned gameplay rule. Pure `Concrete / Slabs / Slab` state now publishes `settled-pour-field` alignment, `settlement-branching` cracks, aggregate, repairs, relief, and wetness through the existing terrain shader and wet-concrete descriptor capabilities. The retained room-facing entry view and three-view headed proof pass all 18 gates with zero console errors/HUD pixels and deterministic 359-owner reset/replay. This promotes `settle-slabs-kit` and `crack-slab-kit`; `SCENE-002` remains `Partially Fixed` for wall/prop/ceiling materials, true reflection, fine ceiling fracture/rubble, remaining silhouettes, and target edge density.
- Progress note (2026-07-17): Fixed the bounded fine-ceiling-fracture and small-settled-rubble slice without adding a natural domain, Nexus core, alias kit, or production kit. Existing Ceiling/Openings, Decay/Cracking, Ground/Rubble, and Fallen Masonry owners now publish localized collapse, aperture-radial crack, breach, rubble, source, settlement, and route-clearance truth. The existing ceiling object now composes 26 parts / 240 triangles with 8 dark fracture paths and 10 chipped edge fragments; the existing furnishing/rubble/debris chain places 3 low collapse-rooted clusters. The dedicated profile passes movement `2.7000`, authoritative `yawDelta=0.32`, `pitchAfter=0.261799`, all descriptor/Nexus/readability/no-HUD/reset-replay/console gates. This promotes only `break-ceiling-kit`, `open-ceiling-kit`, `propagate-crack-kit`, and `collapse-masonry-kit`. Target edge density remains `18.01%` versus roughly `5.15-5.88%` in current proof frames, so `SCENE-002` remains `Partially Fixed`; ceiling surface material identity is next.
- Progress note (2026-07-17): Fixed the bounded ceiling-surface material slice through one `surface` state beneath existing natural Ceiling ownership. Existing chamber/preset, terrain/object material, Nexus graphics/presentation, and Three host paths now realize damp-spalled concrete/brick, aggregate, mineral bloom, moss-darkened moisture, roughness, 2 generated textures, and `0.32` shallow relief across 3 surfaces. Map-like contour cracks, flat shader-only variation, and an intact-only texture pass were rejected. The dedicated profile and canonical/fracture/overlap/floor/wet/success/restart/reconnect regressions pass; no domain, core, alias kit, or production kit was added. `SCENE-002` remains `Partially Fixed` for wall/prop materials, remaining silhouettes, true reflection, and target edge density; that checkpoint selected industrial shelving silhouette identity for the following pass.
- Progress note (2026-07-17): Fixed the bounded industrial-shelving silhouette slice through the existing Storage/Shelves boundary and existing object/wound-mesh/furnishing/graphics/presentation path. The rejected eight-part stand became a 41-part / 680-triangle two-bay rack with 19 tagged stored-object parts; two room placements publish 82 parts, 38 stored parts, and 1,360 triangles. Isolated front/distance/oblique/dark review plus live spawn, approach, and `yawDelta=0.82` side-look frames pass human review, luminance, HUD, descriptor-depth, reset/replay, and console gates. No domain, core, alias kit, or production kit was added, and the separate natural `industrial-shelving-kit` behavior contract remains open. `SCENE-002` stays `Partially Fixed`; shelving corrosion and wear identity is next.
...
- **What the current repo does instead:** The live expedition now streams a bounded furnished service chamber at each authoritative building entry. The chamber combines 77 architectural surfaces, 36 composed props, 17 target texture descriptors, 9 wound-mesh objects, 3 target-local lights, and 22 broken-brick relief patches. `furnishedChamberLayer.ts` realizes 741 deterministic instanced bricks plus 4 standing-water patches, 2 wet-ground films, 6 full-footprint broken-light layers, and a 26-part / 240-triangle irregular collapsed-ceiling object with 8 fracture paths, 10 chipped edge fragments, and 3 small settled-rubble clusters. The broken generator reads as a 27-part / 708-triangle open-frame machine. Two loaded industrial racks now contribute 82 authored parts / 38 stored parts / 1,360 triangles and retain a recognizable two-bay rhythm at spawn, approach, and side look. The upper collapse joins wall and shelter boundaries, the damage radiates into its edge, fallen material belongs to the breach, and the floor presents settled, cracked, damp concrete state. Existing Ceiling truth drives damp-spalled material identity across 3 ceiling surfaces, 2 generated textures, and `0.32` relief. The room is readable and physically deeper, but still below the V2 target in wall/prop surface wear, true reflected-environment response, remaining silhouette detail, and total edge density.
- **Evidence from current code:** `src/features/corridor/domain/ceilingCollapse.ts`, `src/features/render/three/furnishedChamberLayer.ts`, `src/features/render/three/proceduralShaders.ts`, `src/features/render/three/woundMeshObjectRenderer.ts`, `src/protokits/wound-triangle-mesh-domain-kit/index.ts`, `src/protokits/broken-generator-object-kit/index.ts`, `src/protokits/collapsed-ceiling-object-kit/index.ts`, `src/protokits/water-surface-domain-kit/index.ts`, `src/protokits/furnish-chamber-kit/index.ts`, `src/protokits/presets/horror-corridor-preset.ts`, `docs/live-player-harness/latest/report.json`, `docs/live-player-harness/ceiling-material-proof/report.json`, `docs/live-player-harness/ceiling-fracture-proof/report.json`, `docs/live-player-harness/wet-reflection-proof/report.json`, `docs/live-player-harness/generator-silhouette-proof/report.json`, `docs/live-player-harness/ceiling-overlap-proof/report.json`, and `docs/live-player-harness/success-path-proof/report.json`. The material profile passes with `movementDelta=2.2473`, `yawDelta=0.32`, `pitchAfter=0.261799`, readable/HUD-free spawn, route, and upward views, exact descriptor/Nexus agreement, deterministic replay, and zero console errors. The current `34,962 ms` success path reaches Building 3 with score 2 and the matching furnished chamber still present.
- **Likely files to update:** Existing material, surface, object, ceiling, and furnished-chamber realization owners. Add a new kit only if the next behavior cannot be expressed by the installed Nexus core kits or an existing HorrorCorridor-owned domain kit.
- **Suggested fix direction:** Keep `SCENE-002` partial. Industrial shelving silhouette, structural ceiling/wall overlap, fine fracture/settled rubble, ceiling surface identity, wet-concrete floor identity, solo restart, and same-origin Shared Expedition recovery are fixed as bounded subjects; the next visual lane should address shelving corrosion and wear identity, followed by other wall/prop materials, remaining silhouettes, true reflection only with a consuming contract, and edge density.
...
- **Progress note (2026-07-17):** Completed the bounded broken-generator silhouette without adding a domain or alias kit. The existing wound-mesh seam now creates explicit indexed cylinders along all three axes, and the existing generator kit composes 27 parts / 708 outward-CCW triangles. Human review rejected the first 1.55-scale smooth-disc placement as oversized and boiler-like; the retained 1.28-scale spoke version passes the dedicated 1.05-radian headed side-look, ProtoKit mesh smoke, canonical player path, wet-response rerun, and 32.9-second Building-3 success path. This subject is `Fixed`, while `SCENE-002` remains `Partially Fixed` for ceiling, material, reflection, remaining silhouettes, and edge density.
- **Progress note (2026-07-17):** Completed the bounded structural ceiling/wall-overlap subject through the existing core-object chain plus one missing reusable local descriptor. The shared wound-mesh kit gained a generic closed convex slab; `collapsed-ceiling-object-kit` composes 10 parts / 112 outward-CCW triangles, and the room only places it. Four mechanical box/wedge iterations were rejected. The retained three-view headed proof passes movement, upward look, luminance, no-HUD, reset/replay, and console gates. This subject is `Fixed`; fine fracture/rubble and full-room material/edge parity remain partial.
- **Progress note (2026-07-17):** Completed the following fine-fracture and settled-rubble subject through existing natural owners and existing object/material/placement capabilities. `collapsed-ceiling-object-kit` now composes 26 parts / 240 outward-CCW triangles with 8 dark radial paths and 10 chipped edge fragments; three low rubble clusters remain tied to the breach and outside the route. The dedicated headed proof passes all movement/look, descriptor, Nexus, readability, no-HUD, reset/replay, and console gates. The bounded subject is `Fixed`; full-room materials, remaining silhouettes, true reflection, and total edge density remain partial.
- **Progress note (2026-07-17):** Completed the bounded ceiling-surface material subject through one `surface` state under existing natural Ceiling ownership, chamber preset tuning, existing terrain/object material capabilities, Nexus graphics/presentation cores, and Three host realization. Human review rejected topographic-map crack contours, flat shader-only variation, and an intact-only texture pass. The retained generated damp-concrete texture plus `ceiling-ruin` procedural material gives intact and collapsed pieces shared aggregate, mineral, moisture, moss, roughness, and shallow relief. The dedicated three-view proof passes exact descriptor/Nexus state, 3 material surfaces, 2 textures, `surfaceRelief=0.32`, movement/look, readability, no-HUD, reset/replay, and console gates. No domain, core, alias kit, or production kit was added. This subject is `Fixed`; full-room wall/prop materials, silhouettes, reflection, and edge density remain partial.
...
- Current live-player harness result: `docs/live-player-harness/latest/report.json` is a fresh passing headed-Chrome proof with movement, no visible play UI, zero console errors, 77 architectural surfaces, 36 props, 9 mesh objects, 22 masonry-relief patches, 741 bricks, 6 water surfaces/6 broken-reflection layers, two loaded shelving objects / 82 parts / 38 stored parts / 1,360 triangles, 26 collapsed-ceiling parts, 8 fracture paths, 10 edge fragments, 3 rubble clusters, 3 ceiling-material surfaces, and 2 generated ceiling textures. `industrial-shelving-proof`, `wet-reflection-proof`, `generator-silhouette-proof`, `ceiling-overlap-proof`, `ceiling-fracture-proof`, `ceiling-material-proof`, and `success-path-proof` cover their dedicated player paths.
- Current visual-proof blocker: no tooling blocker remains. `SCENE-002` remains `Partially Fixed` because physical masonry, boundary-matched wet response, generator and industrial-shelving silhouettes, structural ceiling overlap, fine fracture/settled rubble, wet-concrete floor identity, and damp-spalled ceiling identity are visible, but the player view still lacks the V2 target's wall/prop surface richness, true reflected environment, detailed remaining object silhouettes, and overall edge density. Dedicated shelving/floor/wet/ceiling/fracture/material/success frames are accepted visual evidence, and thresholds were not lowered.
- Current behavior-proof blocker: the canonical graph has 296 open behavior contracts. Solo restart-after-caught and same-origin two-page reconnect/recovery are proven through real headed flows. Cross-device PeerJS recovery remains unverified, and unrelated contract-only natural behavior remains open.

## 8. Highest-Leverage Fix Order

1. Keep industrial shelving silhouette, structural ceiling/wall overlap, fine fracture/settled rubble, ceiling surface identity, wet-concrete floor identity, solo restart, and Shared Expedition recovery bounded as fixed; resume room work one subject at a time, beginning with shelving corrosion and wear identity, then other wall/prop materials, remaining silhouettes, true reflection only with a consuming contract, and edge density.
2. Add cross-device PeerJS recovery only as a later selected UX slice; reuse the proven Rejoining transaction and request-correlation protocol.
3. Continue closing behavior contracts by natural owner, taking one bounded domain slice at a time from `.agent/domain-service-coverage.md`; do not create alias kits when an installed Nexus core kit already supplies the atomic behavior.
4. Keep `npm run smoke:protokits`, domain checks, the HorrorCorridor harness check, headed player-view proof, restart proof, and continuous success path in the promotion gate.
...
- Networking domain, protocol, and adapters: `src/features/networking/domain/sharedRecovery.ts`, `src/features/networking/protocol/messageTypes.ts`, `src/features/networking/protocol/serializers.ts`, `src/features/networking/protocol/syncSnapshot.ts`, `src/features/networking/peer/createHost.ts`, `src/features/networking/peer/createClient.ts`, `src/features/networking/peer/peerEvents.ts`, `src/components/menus/RecoveryScreen.tsx`, and `scripts/horror-corridor-reconnect-harness.mjs`.
...
Next batch: start from `.agent/goal.md`, `.agent/domain-service-coverage.md`, `docs/HorrorCorridor-Natural-World-Kit-Map.md`, and the shelving-silhouette proof; improve industrial shelving corrosion and wear identity through the retained object profile and existing material/preset/graphics/presentation boundaries, then re-run shelving silhouette, ceiling material/fracture, floor, domain, smoke, headed player-view, reconnect, restart, and continuous success-path gates.
