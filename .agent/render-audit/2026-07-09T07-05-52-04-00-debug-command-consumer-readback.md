# HorrorCorridor Debug Command Consumer Readback
...
`HorrorCorridor` has a visual/render surface through `GameCanvas.tsx`, Three.js renderer creation, camera creation, post-processing setup, maze world rendering, minimap drawing, HUD state, completion routing, and runtime debug frame capture.

## Current render/debug loop

```txt
GameCanvas initializes renderer/camera/post-processing
-> buildMazeWorld creates maze/cube/anomaly/ooze scene objects
-> resizeRenderer keeps camera and composer sized
-> animation loop advances player pose and authoritative state
-> buildReplicatedSnapshot provides render state
-> world.update consumes snapshot facts
...
-> postProcessing.render draws frame
```

## Render readback limitation

Runtime debug frames are useful but still cannot explain command outcomes.
