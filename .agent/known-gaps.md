...
**Goal:** preserve the ordered multiplayer authority gaps while keeping room identity allocation ahead of transport, roster, message, start, and render authority.

- [x] Preserve prior identity, transport, lifecycle, clock, snapshot, movement, rendering, and debug findings.
- [x] Reconcile reservation, collision, ownership, retry, rollback, and visible-hosting findings.
...
explicit Pending/Accepted/Failed UI state: absent
...
```txt
room identity command, generation, and fingerprint
join-code normalization and reservation
PeerJS peer-ID ownership admission
local-bridge identity binding
collision, unavailable, timeout, cancellation, and stale results
bounded retry and supersession
partial resource cleanup and rollback
...
Do not claim globally joinable rooms, collision safety, retry safety, rollback safety, or transport-mode identity parity until the authority and fixtures pass on `main`.