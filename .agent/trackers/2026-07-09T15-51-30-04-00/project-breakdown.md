...
**Run timestamp:** `2026-07-09T15-51-30-04-00`
...
## Next-cut domains required before touching presentation
...
`HorrorCorridor` is playable and render-stable, but its authority seam remains result-poor. `interactionRules.ts` returns only `GameState`, `networkRules.ts` collapses control messages and invalid paths into unchanged state, and `GameCanvas.tsx` uses object identity and implicit reason strings as publish gates.
